"""SeCNV
Usage:
    SeCNV.py <bam_path> <output_path> <ref_file> [--ref=<rn>] [--bin_size=<bl>] [--min_ploidy=<p>] [--max_ploidy=<p>] [--pattern=<pn>]
    SeCNV.py (-h | --help)
    SeCNV.py --version
...
def main(arguments):
    ...
    os.system("python call_cn.py %s %s %s"%(output_path, min_ploidy, max_ploidy))
    stop_time = time.time()
